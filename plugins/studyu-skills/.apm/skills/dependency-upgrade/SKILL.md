...
description: Use when upgrading Flutter, Dart, or package dependencies in the StudyU monorepo with stable-SDK selection, migration review, validation, release, and pull-request gates.
...
Read `references/release-handoff.md` only after all required checks pass and
the final diff is reviewed. It contains stop conditions, versioning, pull
request, and post-merge release gates. Do not create or push a tag before the
pull request merges and explicit production release authorization is received.
